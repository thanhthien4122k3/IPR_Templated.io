...
# Block comment giải thích mã nguồn:
'''
- **Khởi tạo Pygame**: 
  - `pygame.init()`: Khởi động Pygame.
  - `WIDTH, HEIGHT = 1000, 700`: Định nghĩa kích thước màn hình.
  - `screen = pygame.display.set_mode((WIDTH, HEIGHT))`: Tạo cửa sổ hiển thị.
  - `pygame.display.set_caption("...")`: Đặt tiêu đề cửa sổ.

- **Màu sắc và Font**:
  - Định nghĩa các hằng số màu (WHITE, BLACK, GRAY, LIGHT_GRAY).
  - `font = pygame.font.Font(None, 24)`: Tạo font chữ mặc định kích thước 24.

- **Lớp Button**:
  - `__init__`: Khởi tạo nút với tọa độ, kích thước, văn bản, màu, và hành động.
  - `draw`: Vẽ nút với viền và văn bản.
  - `is_clicked`: Kiểm tra xem chuột có nhấn vào nút không.

- **Lớp Shape**:
  - `__init__`: Khởi tạo hình khối với loại, tọa độ, kích thước, màu, v.v.
  - `draw`: Vẽ hình khối (vuông, tròn, tam giác, đường thẳng) với hiệu ứng xoay và trong suốt.
  - `is_point_inside`: Kiểm tra xem chuột có nằm trong hình khối không (sau khi xoay).
  - Xóa `is_near_resize_point` vì chức năng kéo cạnh/góc đã chuyển sang đường viền.

- **Lớp Editor**:
  - `__init__`: Khởi tạo danh sách hình khối và các nút trong thanh công cụ. Xóa nút "Pick Color".
  - `add_shape`: Thêm hình khối mới.
  - `set_selected_property`: Thay đổi thuộc tính của hình khối được chọn.
  - `is_near_border`: Kiểm tra chuột có gần cạnh hoặc góc của đường viền không.
  - `handle_events`: Xử lý sự kiện (thoát, kéo thả, điều chỉnh kích thước qua đường viền). Xóa logic chọn màu.
  - `draw`: Vẽ màn hình, bao gồm thanh công cụ, hình khối, và đường viền bao quanh hình khối được chọn.

- **Vòng lặp chính**:
  - `while True`: Chạy vòng lặp chính, gọi `handle_events` và `draw` liên tục.
  - `clock.tick(60)`: Giới hạn tốc độ khung hình ở 60 FPS.

- **Chỉnh sửa gần đây (Yêu cầu 1)**:
  - Xóa chức năng chọn màu:
    - Xóa các biến liên quan (`COLOR_PICKER_WIDTH`, `color_picker_active`, v.v.).
    - Xóa mã tạo bảng màu gradient và thanh trượt độ sáng.
    - Xóa nút "Pick Color" trong danh sách nút.
    - Xóa logic chọn màu trong `handle_events` và `draw`.

- **Chỉnh sửa gần đây (Yêu cầu 2)**:
  - Thêm đường viền hình chữ nhật:
    - Trong `draw`, vẽ đường viền bao quanh hình khối được chọn với `pygame.draw.rect`.
    - Đường viền có khoảng cách 5 pixel xung quanh hình khối (tổng cộng rộng hơn 10 pixel so với hình).
  - Chuyển chức năng điều chỉnh kích thước sang đường viền:
    - Thêm `is_near_border` để kiểm tra chuột gần cạnh/góc của đường viền.
    - Cập nhật `handle_events` để điều chỉnh kích thước hình khối dựa trên việc kéo đường viền.
'''